...
<template>
  <template v-if="props.placement === 'inline'">
    <div class="w-full">
      <div
        class="w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/60 backdrop-blur p-3"
      >
        <div
        ...
        >
          <div class="min-w-0">
            ...
            <p v-if="subtitle" class="truncate text-xs text-slate-500">
              {{ subtitle }}
            </p>
          </div>
  ...
  <template v-else>
    <div
      class="fixed right-3 md:right-6 top-1/2 -translate-y-1/2 transform z-40"
    >
      <div
