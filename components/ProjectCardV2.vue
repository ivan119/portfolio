...
<template>
  <article
    ...
    :style="{
      ...
      '--gradient-from': getGradientColors(project.categories)[0],
      '--gradient-to': getGradientColors(project.categories)[1]
    }"
  >
    <NuxtLink
      :to="project.link"
    ...
    >
      ...
      <div class="absolute top-2 right-2 z-10">
        <span
          ...
          :style="{
            ...
            color: 'white',
            boxShadow: `0 2px 4px ${getCategoryColor(project.categories)}40`
          }"
        >
      ...
      <div class="relative p-3">
        <div 
          ...
          :style="{
            background: `linear-gradient(135deg, ${getCategoryColor(project.categories)}20, transparent)`
          }"
        ></div>
        <i 
      ...
      <!-- Title -->
      <h2 
        class="text-base font-semibold text-gray-900 dark:text-white px-3 mb-1 transition-all duration-300 group-hover:text-[var(--gradient-from)] group-hover:translate-x-1"
      >
        {{ project.title }}
      </h2>
      ...
      <div class="flex flex-wrap gap-1 px-2 pb-2">
        <span
          ...
          :style="{
            background: `linear-gradient(135deg, ${getCategoryColor(project.categories)}15, ${getCategoryColor(project.categories)}05)`,
            color: getCategoryColor(project.categories),
            boxShadow: `0 1px 2px ${getCategoryColor(project.categories)}20`
          }"
        >
          <i :class="[tag.icon, 'text-xs']" :style="{ color: getCategoryColor(project.categories) }"></i>
          {{ tag.name }}
        </span>
      </div>
      ...
      <!-- Hover Indicator -->
      <div 
        class="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-[var(--gradient-from)] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"
      ></div>
    </NuxtLink>
  </article>
</template>
