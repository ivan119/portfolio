...
const config = ref({
  particleColor: "#6366f1", // Indigo color for dark mode particles
  lightModeColor: "#4f46e5", // Indigo color for light mode
  particleCount: 80, // Reduced particle count for cleaner look
  connectionDistance: 12, // Increased connection distance
  connectionOpacity: 0.15, // Reduced opacity for softer connections
  connectionAnimDuration: 1.2, // Slightly slower animations
  pulseStrength: 1.5, // Reduced pulse strength for subtler effect
});
...
onMounted(() => {
  ...
  const fragmentShader = `
    ...
    void main() {
      ...
      vec3 darkColor1 = vec3(0.05, 0.05, 0.2);  // Dark blue
      vec3 darkColor2 = vec3(0.2, 0.1, 0.4);    // Purple
      vec3 darkColor3 = vec3(0.1, 0.2, 0.5);    // Blue

      vec3 lightColor1 = vec3(0.9, 0.95, 1.0);  // Almost white
      vec3 lightColor2 = vec3(0.8, 0.85, 1.0);  // Light blue
      vec3 lightColor3 = vec3(0.7, 0.75, 1.0);  // Slightly darker blue

      float glow = sin(vUv.x * 4.0 + time * 0.4) * 0.5 + 0.5;
      float glow2 = cos(vUv.y * 5.0 + time * 0.2) * 0.5 + 0.5;
...
// Update particle movement to be slower and more subtle
